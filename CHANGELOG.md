...
- Added owner-only permissions for local runtime data as a follow-up to that PR: `data/`, `data/documents/`, SQLite files, sidecars, and uploaded documents now use private POSIX permissions where supported. This reduces accidental exposure to other local users while clarifying that same-user disk access still requires FileVault, encrypted volumes, or future SQLCipher support for raw-file protection.
